...
# ===== Pulling in the Global Site-IM R dataset ======
datim <- readRDS("./Genie_TSD_v2/RawData/Site_IM_All_OUs_Genie_FY18Q4_11_16.rds")
  ...
  ou <- count(datim, operatingunit)
  natou <- count(natx, operatingunit)
...
for (x in oulist) {  
  ...
  datimx <- datimx %>% 
    mutate(mechanismid = as.character(mechanismid),
           approvallevel = as.character(approvallevel)) %>% 
