...
# Keeping only data for country of interest
sims <- simsx %>% filter(OperatingUnit %in% ou)

ouxx <- ifelse(ou=="Democratic Republic of the Congo", 
               "DRC", 
               ifelse(ou=="Cote d'Ivoire", 
               oux , 
               gsub(" ", "", ou, fixed = TRUE)))


# Keeping only data for country of interest
ea <- eax  %>% filter(OU %in% ouxx)
...
ea1 <- ea %>% 
  ...
  filter(datim_snu %in% c("_MILLA", "", PSNUlist) | is.na(datim_snu)) %>% 
...
# Adding PSNU name incase ea data names are different
 ea_psnu <- fact %>% 
  select(PSNUuid, PSNU) %>%
  unique() %>% 
...
# excluding entities without MER data
  filter((SNU1 %in% SNU1list &
         PSNUuid %in% PSNUlist &
         orgUnitUID %in% sitelist) | 
         !is.na(ART_UE_fy15_nat           ) | 
         !is.na(ART_UE_fy16_nat           ) |
         !is.na(ART_UE_fy17_nat           ) |
         !is.na(HTC_UMB_TST_UE_fy17_nat   ) |
         !is.na(CBCTS_LOADED_TOT_fy17_nat ) |
         !is.na(CBCTS_LNKG_EXP_fy17_nat   ) |
         !is.na(CBCTS_RTNADHR_EXP_fy17_nat) |
         !is.na(CBCTS_MEDCARE_EXP_fy17_nat)   )
