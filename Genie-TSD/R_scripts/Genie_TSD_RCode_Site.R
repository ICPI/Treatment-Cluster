...
# Pulling in the UNAIDS national-level Epidemiological estimates
unaids <- readRDS("PLHIV15_17_UNAIDS.rds")


# Looking for the NAT_SUBNAT dataset in folder
impattfile <- list.files("."
                         , pattern="NAT_SUBNAT")
...
} else {"Thanks for using the NAT_SUBNAT dataset... woot woot!!"}

if (grepl(".zip", impattfile)) {
  "Zippty doo... you're using a Zipped file!!"
  n_file <- unzip(impattfile, list = TRUE) %>% .$Name
  #extract file from zipped folder
  unzip(impattfile, exdir = ".")} else {"You're using the unzipped file... working hard!"
    n_file <- impattfile}
...
if(!all(natvars %in% nfoonames)){
  ...
  ":) You set the NAT_SUBNAT Genie free! Woo hoo!!"
}
...
genie_filelist <- intersect(list.files(".",pattern = "(Genie|genie)"), 
...
for (x in genie_filelist) {
  g_file <- unzip(x, list = TRUE) %>% .$Name
  #extract file from zipped folder
  unzip(x, exdir = ".")
  
  # Rough data pull to get variable names and assign datatype
  foo <- read_tsv(file=g_file, 
                  col_names = TRUE)
